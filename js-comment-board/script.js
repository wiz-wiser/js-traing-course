...
const inputHandler = () => {
	...
	const maxCNrChars = MAX_CHARS;

	// determine number of characters user has currently typed
	const nrCharsTyped = textareaEl.value.length;

	// calculate number of characters left
	const charsLeft = maxCNrChars - nrCharsTyped;
...
const submitHandler = (event) => {
	...
	// send feedback item to server - fetch is async
	fetch(`${BASE_API_URL}/feedbacks`, {
		...
		.then((response) => {
			if (!response.ok) {
				console.log("Not okay - something went wrong");
				return;
			}
...
const clickHandler = (event) => {
	...
	//determine if use intended to upvote or expand
	const upvoteIntention = clickedEl.className.includes("upvote");

	//run the logic for each option
	if (upvoteIntention) {
		//get the closest upvote
		const upvoteBtnEl = clickedEl.closest(".upvote");

		//disable upvote button - stop double click and spam
		upvoteBtnEl.disabled = true;

		//select the upvote count el
		const upvoteCountEl = upvoteBtnEl.querySelector(".upvote__count");

		//get currely displayed upvote count as number (+)
		let upvoteCount = +upvoteCountEl.textContent;

		//set upvote count incremented by one - have to have ++ first in this case to increment by one before its assigned
...
fetch(`${BASE_API_URL}/feedbacks`)
	...
	.then((data) => {
		...
		//iterate over each element and render in list
...
const clickHandler2 = (event) => {
	...
	//stop function if clik happens in list, but outside buttons
	if (clickedEl.className === "hashtags") return;

	//extract company name after the hashtag (0)
